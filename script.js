document.addEventListener('DOMContentLoaded', function () {
    ...
    const chatRef = campaignRef.collection('chat');
    // e assim por diante

    function listenToCampaign() {
        campaignRef.get().then(doc => {
            if (doc.exists) {
                const campaignData = doc.data();
                console.log("Dados da campanha:", campaignData);
                // Aqui você pode atualizar a UI com o nome da campanha etc.
            } else {
                alert("Campanha não encontrada.");
            }
        });

        // Escuta para itens compartilhados com o jogador
        itemsRef.onSnapshot(snapshot => {
            snapshot.forEach(doc => {
                const item = doc.data();
                console.log("Item recebido:", item);
                // Aqui você pode mostrar ou adicionar o item no inventário do jogador
            });
        });

        // Escuta para mensagens ou eventos
        chatRef.orderBy('timestamp', 'desc').limit(50).onSnapshot(snapshot => {
            snapshot.forEach(doc => {
                const msg = doc.data();
                console.log("Mensagem recebida:", msg);
                // Exibir no chat do jogador, por exemplo
            });
        });
    }
    listenToCampaign();
